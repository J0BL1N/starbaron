...
--   here).
-- Date      : 2026-08-13
-- Scope     : 3 new tables + 3 indexes + RLS (enable + force) + owner
...
--    (owned_planets.owner_id precedent, 0001). created_at/updated_at are
--    DB-filled operational metadata (0013 convention) — the deterministic
--    sim keeps its own createdAt.
-- ---------------------------------------------------------------------
...
--    (D7); target is nullable JSONB ({kind,id}, null for 'return'); the
--    parent fleet FK cascades (0004 attack_members.attack_id precedent).
-- ---------------------------------------------------------------------
create table public.fleet_order (
  ...
  target     jsonb,              -- FleetOrderTarget {kind,id} | null ('return' carries no target)
...
--    waypoints is the JSONB waypoint array; total_distance_pc is numeric
--    (>= 0 — route totals are always non-negative); departure_at must be
--    strictly before arrival_at (routes.ts guarantees arrival > departure).
--    The id is a persistence key — the sim route carries no id field (the
--    caller supplies one at persist time, like owned_planets.id). Parent
--    FK cascades like fleet_order.
-- ---------------------------------------------------------------------
