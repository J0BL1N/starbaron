...
| 7 | **Binary quirk at level-0 Trade Hub — Option A/B PENDING Jay** | P2-T04-A §9.1 / T04-D | `applyQuirk` runs before any level gate, so a level-0 Trade Hub on a binarySystem planet yields baseline ×1.1. **Option A (baseline-trait, current code, recommended)** keeps it; **Option B (structure-gated, needs `levels.tradeHub ≥ 1`)** is a wiring change + regression test. **Jay's decision needed before P3** — flagged, not silently resolved |
...
4. **Binary quirk level-0 Option A/B PENDING Jay** — see §7 item 7; carried from T04-D as the phase's one open design decision, required before P3.
