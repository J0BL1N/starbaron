...
| Planet quirks / visual / description | — | stay **client-side** (deterministic generator, `makePlanet(entry).generate()`). Only the two combat-relevant quirks (`massiveWorld`, `denseCore`) reach the server — via a small balance seed, §5.3 / D4 |
...
create table public.players (
  ...
  credits          double precision not null default 1000 check (credits >= 0),   -- STARTER_CREDITS
  alloys           double precision not null default 0    check (alloys >= 0),    -- STARTER_ALLOYS
...
create table public.owned_planets (
  ...
  baseline_income_per_sec     double precision not null,
  population_cap_multiplier   double precision not null,
  distance_pc                 double precision,                -- catalogue, denormalised (travel time, §5.2)
  ...
  garrison                    double precision not null default 0 check (garrison >= 0),
  fleet                       double precision not null default 0 check (fleet >= 0),
  structure_levels            jsonb not null                  -- Record<StructureId, number> (§1.3)
);
...
create table public.attacks (
  ...
  join_window_seconds int  not null default 7200,             -- DESIGN §5 "launch window (e.g. 2h)"
  travel_seconds      int  not null,                          -- distancePc × 1 min, floor 600, cap 172800 (§5.2)
...
create table public.attack_members (
  ...
  soldiers_committed double precision not null check (soldiers_committed >= 0),  -- the permanent war cost (§4a)
...
| `attack_members` | participants (launcher + members) + target owner; inbound → all authenticated (mirror `attacks`) | INSERT via `join_attack` |
...
3. **`SELECT … FOR UPDATE` on the target row** in resolve (and `FOR SHARE` in launch) — only one resolution/launch proceeds per attack/planet; the loser reads the committed result (§5.3).
...
**Recommendation:** a `SECURITY DEFINER` `resolve_due_attacks()` (and an internal `resolve_attack(attack_id)`) — a pure Postgres function, invoked lazily at the top of read RPCs (`get_galaxy_state`, `get_attack`, login sync) — the idle-genre standard.
...
|---|---|---|
| `0001_players_owned_planets.sql` | `players`, `owned_planets`, both uniqueness indexes, derived-field CHECKs | RLS on both; SELECT policies; REVOKE anon/PUBLIC; GRANT SELECT + EXECUTE-to-authenticated |
| `0002_meta_tables.sql` | `notifications`, `seasons`, `leaderboard_snapshots` (needed by later RPCs) | RLS + grants per §3.2 |
...
| `0006_game_config_seed.sql` | `game_config` + seed of the draft tunables **and the small combat-balance constants** (structure base effects, turret/DP coefficients, `effectiveLevel` half-after-10, `massiveWorld`/`denseCore` multipliers) — the server-authoritative resolve math (D4) | service_role-only read |
...
No migration is applied in this subtask. No `supabase/` directory exists in the repo today (verified).
...
| **D5** | Attack resolution engine | (1) **DB function + lazy on-read**; (2) Edge Function; (3) `pg_cron` | **(1)** — atomic single transaction, no cold starts, idle-genre standard (results consistent with what's displayed), no background worker to babysit on free tier. `FOR UPDATE` handles the resolve race |
...
*Prepared by OpenCode (deepseek-v4-flash) for P3-T01-A. Only `docs/P3_T01_A_AUDIT.md` created. No commit made, no work on `main`, no Supabase CLI invocation.*
