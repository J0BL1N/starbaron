...
4. Purity: no nondeterministic APIs/module mutable state/wall-clock; no `any`; imports ⊆ player/transfer + combat/resolution + combat/casualties + combat/conquest-cost + world/api + ui/validate + stdlib + (CONTRACT CORRECTION — pure modules the fix legitimately requires): planets/hash (fnv1a ids), world/identity (BodyId, type-only), world/reconstruct (UniverseState, type-only), structures/data (STRUCTURE_IDS — the surviving-structures report), player/ownership (OwnershipRecord — the finding-2 required input), player/types (type-only); banned comment tokens absent; tables frozen.
