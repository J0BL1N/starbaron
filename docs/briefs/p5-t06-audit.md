...
AUTHORISED SCOPE: ONLY the two new files. AUDIT TARGET = the CURRENT state of src/sim/fleet/render-state.ts + tests/render-state.test.ts on staging (HEAD). The implementation commits (feat + audit-fix amends) modify ONLY these two files. Docs commits OUT OF SCOPE. No existing file modified.
