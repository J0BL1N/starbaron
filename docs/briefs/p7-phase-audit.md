...
G. GAP SCAN vs the roadmap's P7 subtask list (attack orders; invasion fleet credit cost + population recruitment; attack power + defense power; defense structures + population garrison; committed attackers lost appropriately + defender losses; base cost + empire-size escalation; ownership transfer + structures; reject attack/conquest + backend enforcement; winner/loser + ship losses; scenario generation + deterministic replay; incoming attack + ETA) — flag any subtask with NO implementation surface.
