...
AUDIT TARGET: the whole Phase 2 feature set on staging at the CURRENT HEAD (src/sim/player/*.ts + tests/*.test.ts + supabase/migrations/0016_ownership_canonical.sql + supabase/tests/10_home_claim_atomicity.sql — the phase-fix commits through round 5 addressed your previous findings) consistently; no module hand-strings ids; parseCanonicalId used wherever validation happens.
