...
| **P3-T03** Conquest math | **-A** | Audit: AP/DP ratio, 4 outcomes, militia 15%, garrison/deploy model | **Complete** — docs/P3_T03_A_AUDIT.md; **27-constant resolver-vs-DESIGN §5a audit** (5 constant classes, zero drift except the deliberate B4 v1 exclusion), boundary-inclusivity (`>=` on all upper boundaries), AP/DP computation verified (effectiveLevel server-side present), estimator parity matrix, casualty semantics, 10 edge cases, decisions D1–D6 |
...
| **P3-T05** Fortification + war-weariness | **-A** | Audit: 10× ceiling curve, war-weariness +20%/24h per player | **Complete** — docs/P3_T05_A_AUDIT.md (6b77016); 10× = emergent (B3), weariness counter LIVE, B1 server-build gap flagged; **Jay: build RPC approved** |
| | **-B** | Implement: fortification cost curve (alloy-heavy, escalating), war-weariness counter | **Complete** — **0012_fortification_commitment.sql (build_structure RPC B1-approved, source FOR UPDATE + garrison/fleet guards + source_planet_name in launch/join, resolve_attack uniform survivor return clamped B4/B5, get_attack roster weariness B7)** + 02/04/05 re-seeded + 06 suite written + estimator mirrors (canDeploy/survivors/caps) + WarWearinessPanel; 591 tests, tsc/build/lint 0, Codex PASS (1 MINOR fixed + comment); **0012 apply to live PENDING Jay authorisation** |
